...
def print_board(board):
    ...
    for row_idx in range(len(board)):
        print(main.make_str_from_row(board, row_idx))
...
def play_game(players, board, words):
    ...
    """
    num_remaining = main.num_words_on_board(board, words) - len(found_words)
    player_num = 0
    while num_remaining > 0:
        ...
        if main.is_valid_word(words, guess) and main.board_contains_word(board, guess) and \
            not guess in found_words:
            main.update_score(players[player_num % len(players)], guess)
...
words = main.read_words(words_file)
words_file.close()

# Load the board.
board_file = askopenfile(mode='r', title='Select board file')
board = main.read_board(board_file)
board_file.close()
